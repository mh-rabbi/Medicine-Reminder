...
//----------------updated medicine image home screen ------------------------//
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';
import 'package:medicine_reminder/constants/app_constants.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: AppColors.primary,
        onPressed: () async {
          // Open add alarm screen
          final result = await Navigator.of(context).push(_bottomUpRoute(const AddAlarmScreen()));
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              // Medicines/Alarms list
              Expanded(
                child: AppConstants.alarms.isEmpty
                    ? Center(
                  child: Column(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return AlarmCard(
                      ...
                      medicineImagePath: alarm.medicineImagePath,
                      foodTiming: alarm.foodTiming,
                      onTap: () async {
                        // Navigate to edit screen with existing alarm data
                        final result = await Navigator.of(context).push(
                          _bottomUpRoute(AddAlarmScreen(existingAlarm: alarm)),
                        );
                      ...
                      onDelete: () async {
                        ...
                        if (confirm == true) {
                          // Cancel all scheduled alarms for this medicine
                          for (int i = 0; i < alarm.times.length; i++) {
                            await Alarm.stop((alarm.id ?? 0) + i * 100000);
                          }
